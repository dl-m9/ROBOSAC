...
class FaFModule(object):
    ...
    def amongus_step(self,data,batch_size,invert_gt=False,return_result=False,ego_only=False,self_result=None):
        bev_seq = data['bev_seq']
        labels = data['labels']
        ...
        if self_result is not None:
            print("Using self_pred cls label")
            labels = self_result['cls']
                

        # pert.requires_grad = True
        
        result = self.model(bev_seq, trans_matrices, num_agent, batch_size=batch_size, pert=pert, confidence=confidence, no_fuse=ego_only)

        labels = labels.view(result['cls'].shape[0],-1,result['cls'].shape[-1])

        N = bev_seq.shape[0]
        # print("result['cls'].shape",result['cls'].shape)
        # print("result['loc'].shape",result['loc'].shape)
        # print("labels.shape",labels.shape)
        loss_collect = self.loss_calculator(result,anchors,reg_loss_mask,reg_targets,labels,N,invert_gt=invert_gt)
        loss_num = loss_collect[0]
        if loss_num == 3:
            loss_num,loss, loss_cls,loss_loc_1,loss_loc_2 = loss_collect
        elif loss_num ==2:
        ...
        if self.config.pred_type in ['motion','center'] and not self.only_det:
            ...
            if return_result:
                return loss.item(),loss_cls.item(),loss_loc.item(),result
            else:
                return loss.item(),loss_cls.item(),loss_loc.item()
    ...
    def cls_predict(self,data,batch_size,no_fuse=False):
        bev_seq = data['bev_seq']
        trans_matrices = data['trans_matrices']
        num_agent = data['num_agent']
        pert = data['pert']
        ...
        result = self.model(bev_seq, trans_matrices, num_agent, batch_size=batch_size, pert=pert, no_fuse=no_fuse, ego_agent=ego_agent)
        
        return result['cls']

    def cls_loss_calculator(self,result,anchors,reg_loss_mask,reg_targets,labels,N,motion_labels = None,motion_mask=None, invert_gt=False, ego_loss_only=False, ego_agent=1,adv_method='pgd'):
        
        
        # test_perturb_init = torch.zeros((1024, 3)).to(device).float()
        # test_perturb_init.requires_grad = True
        # print(test_perturb_init.shape)
        # print(labels.dtype)
        if adv_method == 'pgd' or adv_method == 'bim':
            if invert_gt:
                # print("labels is inverted")
                ...
                # print(labels)
                labels = labels.float()
            if not ego_loss_only:
                loss_cls = torch.sum(self.criterion['cls'](result['cls'],labels)) / N
            else:
                # print("ego loss only, calculated on agent 1")
            ...
            def cw_loss(result, labels):
                kappa = 0
                c = 0.1
                labels = labels.view(-1, 2).type(torch.float)
                # Define f-function
                def f(x) :
                    x = x.view(-1, 2)
                    outputs = x
                ...
                # We don't implement the loss of constraints here.
            ...
            if not ego_loss_only:
                loss_cls = torch.sum(cw_loss(result['cls'],labels)) / N
            else:
                # print("ego loss only, calculated on agent 1")
                loss_cls = torch.sum(cw_loss(result['cls'][ego_agent],labels[ego_agent]))
            
        return loss_cls
    
    @torch.no_grad()
    def cal_forward_time(self,data,batch_size,iterations=100):
        ...
        times = torch.zeros(iterations)     # array for saving inference time
        with torch.no_grad():
            for iter in range(iterations):
                # forward
                starter.record()
                
                _ = self.model(bev_seq, trans_matrices, num_agent, 
                batch_size=batch_size, pert=pert, no_fuse=no_fuse, 
                unadv_pert=unadv_pert, attacker_list=attacker_list,
                ...
                curr_time = starter.elapsed_time(ender)  # cal time
    ...
    def cls_step(self,data,batch_size,invert_gt=False,return_result=False,no_fuse=False,self_result=None,ego_loss_only=False,ego_agent=1,adv_method='pgd', cp_guard_defense=False):
        ...
        # print("result['loc'].shape",result['loc'].shape)
        # print("labels.shape",labels.shape)
        loss_cls = self.cls_loss_calculator(result,anchors,reg_loss_mask,reg_targets,labels,N,invert_gt=invert_gt,ego_loss_only=ego_loss_only,ego_agent=ego_agent,adv_method=adv_method)
        # print("loss cls:", loss_cls)
    ...
    def predict_all(self, data, batch_size, validation=True, num_agent=5, adv_method=None, cp_guard_defense=False):
        ...
        trial_agent_id = data['trial_agent_id']
        attacker_list = data['attacker_list']
        eps = data['eps']
        ego_agent = data['ego_agent']
        ...
        if self.config.flag.startswith("when2com") or self.config.flag.startswith(
            ...
            # timestamper
            # starter, ender = torch.cuda.Event(enable_timing=True), torch.cuda.Event(enable_timing=True)
            # times = torch.zeros(50)     # array for saving inference time
            # with torch.no_grad():
            ...
            #         _ = self.model(
            #             bev_seq, trans_matrices, num_all_agents, 
            #             batch_size=batch_size, pert=pert, no_fuse=no_fuse, 
